...
def rollout_policy_fn(board):
    ...
    for i in range(len(moves)):
        moves[i] = moves[i].uci()
    action_probs = np.random.rand(num)
    return zip(moves, action_probs)
...
class MCTS(object):
    ...
    def _evaluate_rollout(self, state, limit=1000):
        ...
        for i in range(limit):
            ...
            a_p = dict(action_probs)
            try:
                max_action = max(a_p.items(), key=itemgetter(1))[0]
            except:
                continue
            move = chess.Move.from_uci(max_action)
            state.push(move)
            if state.result(claim_draw = True) != '*':  # tie
...
class MCTSPlayer(object):
    ...
    def __init__(self, c_puct=5, n_playout=10):
