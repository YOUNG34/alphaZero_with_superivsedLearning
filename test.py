...
class GUI(object):
    def get_player(self, config):
        from src.chess_zero.agent.model_chess import ChessModel
        from src.chess_zero.lib.model_helper import load_best_model_weight
        model = ChessModel(config)
        if not load_best_model_weight(model):
            raise RuntimeError("Best model not found!")
        return ChessPlayer(config, model.get_pipes(config.play.search_threads))

    def info(self, depth, move, score):
